...
# Salto fijo de frames para videos de dron (0.7 m/s)
# Con 30fps, analiza 1 frame por cada ~33 cm recorridos
SKIP_FRAMES_VIDEO = 10
...
def main_process(imagen_pil, save_to_disk=False):
    ...
    nombres_extra = {
        0: "Persona", 15: "Gato", 16: "Perro", 17: "Caballo",
        18: "Oveja", 19: "Vaca", 39: "Botella/Basura"
    }
...
def run_inference_seg(img_rgb, size=384):
    res = modelo_seg(img_rgb, conf=0.4, classes=[0, 15, 16, 17, 18, 19, 39], imgsz=size, verbose=False)
    ...
    try:
        ...
        for frame in input_container.decode(video=0):
            ...
            if total_frames > 0:
                ...
                bar.progress(pct, text=f"Procesando: {int(pct*100)}% (frame {count}/{total_frames})")
...
if opcion == "🚀 Identificador":
    ...
    if img_in:
        ...
        if st.button("🔍 COMENZAR ANÁLISIS"):
            res, np_count, ns = main_process(pil_in, save_to_disk=True)
            with col2:
                st.image(res, caption="Resultado")
                st.metric("Detecciones", f"{np_count} Plantas | {ns} Alertas")
